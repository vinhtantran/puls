PULS.nonfd<-function(toclust, method=c("pam", "ward"),intervals=c(0,1),spliton=NULL,
               distmethod = "euclidean",
               labels=row.names(toclust),
               nclusters,
  ...
  if(!is.data.frame(toclust)){
    cat("has to be a data frame with columns are timeline and rows are subjects, with values at cells")
    return(0)
  }

  ## Make variables that are simple derivatives of inputs that will be used a lot.
  labs<-labels
  nvars<-nrow(intervals)  #Now number of subregions so number of rows in intervals matrix - drop this?
  nobs<-length(labs)
  ...
  for (j in 1:nsub){
    dsubs[,,j]=as.matrix(distmatrix(toclust,subrange=intervals[j,], distmethod))
    dsubs[,,j] <- as.matrix(dist(toclust, ))
  }
...
distmatrix=function(yfd=yfd,subrange=subrange, distmethod){ #Eventually modify to allow unions of subintervals to be defined
